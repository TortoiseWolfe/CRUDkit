...
async function mockGeolocation(page: Page, latitude = 51.505, longitude = -0.09) {
...
test.describe('Geolocation Map Page', () => {
  ...
  test('should display location button when showUserLocation is enabled', async ({ page }) => {
    await page.goto('/map');
  ...
  test('should show consent modal on first location request', async ({ page }) => {
    ...
    await expect(page.getByText(/would like to use your location/i)).toBeVisible();
  });
  ...
  test('should show marker popups on click', async ({ page }) => {
    ...
    await expect(page.locator('.leaflet-popup-content')).toContainText('Test Marker');
  });
  ...
  test('should handle dark mode theme', async ({ page }) => {
    ...
    const controlBackground = await page.locator('.leaflet-control').evaluate((el) => {
  ...
  test('should handle accessibility requirements', async ({ page }) => {
    ...
    await expect(mapContainer).toHaveAttribute('aria-label', /interactive map/i);
