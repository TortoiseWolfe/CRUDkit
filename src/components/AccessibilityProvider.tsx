...
export default function AccessibilityProvider({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    ...
    // Apply settings
    root.style.setProperty('--font-scale-factor', scaleFactors[savedFontSize]?.toString() || '1.5');
    root.style.setProperty('--base-line-height', lineHeights[savedLineHeight] || '1.5');
    root.style.setProperty('--base-font-family', fontFamilies[savedFontFamily] || fontFamilies['sans-serif']);
    
    document.body.style.lineHeight = lineHeights[savedLineHeight] || '1.5';
    document.body.style.fontFamily = fontFamilies[savedFontFamily] || fontFamilies['sans-serif'];
  }, []);
