...
module.exports = {
  ...
  helpers: {
    ...
    assertComponentStructure: (component) => {
      const requiredFiles = ['index.tsx', `${component.name}.tsx`, `${component.name}.test.tsx`, `${component.name}.stories.tsx`];
    ...
    createTestComponent: (name, category = 'atomic', isCompliant = true) => {
      ...
      if (isCompliant) {
        ...
        structure[`${base}/${name}.tsx`] = `export default function ${name}() { return <div>${name}</div>; }`;
        structure[`${base}/${name}.test.tsx`] = `describe('${name}', () => { it('renders', () => {}); });`;
        structure[`${base}/${name}.stories.tsx`] = `export default { title: '${category}/${name}' };`;
      } else {
