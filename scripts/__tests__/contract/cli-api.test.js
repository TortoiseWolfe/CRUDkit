...
const specPath = path.join(__dirname, '../../../specs/002-component-structure/contracts/cli-api.yaml');
let apiSpec;
try {
...
describe('CLI API contract tests', () => {
  describe('audit endpoint contract', () => {
    it('should accept parameters matching /audit schema', () => {
      ...
      const auditSchema = apiSpec.paths['/audit'].post.requestBody.content['application/json'].schema;
    ...
    it('should return response matching AuditReport schema', () => {
      ...
      assert.ok(Array.isArray(result.components) || Array.isArray(result.nonCompliant),
        'Should have components or nonCompliant array');

      // Validate summary structure
      if (result.summary) {
        assert.ok(typeof result.summary.total === 'number', 'Summary should have total');
        assert.ok(typeof result.summary.compliant === 'number', 'Summary should have compliant');
        assert.ok(typeof result.summary.nonCompliant === 'number', 'Summary should have nonCompliant');
        assert.ok(typeof result.summary.complianceRate === 'number', 'Summary should have complianceRate');
      }
    });
  ...
  describe('migrate endpoint contract', () => {
    it('should accept parameters matching /migrate schema', () => {
      ...
      const migrateSchema = apiSpec.paths['/migrate'].post.requestBody.content['application/json'].schema;
    ...
    it('should return response matching MigrationResult schema', () => {
      ...
      // Validate response structure
      assert.ok(typeof result.success === 'boolean' || result.migrated !== undefined,
        'Should have success or migrated count');

      if (result.migrated !== undefined) {
        assert.ok(typeof result.migrated === 'number', 'Migrated should be number');
      }
  ...
  describe('validate endpoint contract', () => {
    it('should accept parameters matching /validate schema', () => {
      ...
      const validateSchema = apiSpec.paths['/validate'].post.requestBody.content['application/json'].schema;
    ...
    it('should return proper exit codes', () => {
      ...
      if (validResult.exitCode !== undefined) {
        assert.ok(validResult.exitCode === 0 || validResult.exitCode === 1,
  ...
  describe('scaffold endpoint contract', () => {
    it('should validate required parameters for /scaffold', () => {
      ...
      const scaffoldSchema = apiSpec.paths['/scaffold'].post.requestBody.content['application/json'].schema;

      // Check required fields
      assert.ok(scaffoldSchema.required.includes('name'), 'Name should be required');
      assert.ok(scaffoldSchema.required.includes('category'), 'Category should be required');
  ...
  describe('common response fields', () => {
    it('should include timestamp in responses', () => {
      ...
      if (result.timestamp) {
        ...
        assert.ok(timestamp instanceof Date && !isNaN(timestamp),
