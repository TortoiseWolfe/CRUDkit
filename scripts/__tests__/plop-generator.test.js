...
describe('plop component generator', () => {
  describe('generator configuration', () => {
    it('should export a function for plop', () => {
      assert.strictEqual(typeof plopfile, 'function', 'plopfile should export a function');
    });
    ...
    it('should register component generator', () => {
      ...
      plopfile(mockPlop);
      assert.ok(mockPlop.generators.component, 'Should register component generator');
    });
    ...
    it('should have required prompts', () => {
      ...
      assert.ok(promptNames.includes('name'), 'Should prompt for component name');
    ...
    it('should have actions to create files', () => {
      ...
      assert.ok(generator.actions.length >= 4, 'Should have at least 4 actions (4 files)');

      const paths = generator.actions.map(a => a.path);
      assert.ok(paths.some(p => p.includes('index.tsx')), 'Should create index.tsx');
      assert.ok(paths.some(p => p.includes('.tsx') && !p.includes('index')), 'Should create component file');
      assert.ok(paths.some(p => p.includes('.test.tsx')), 'Should create test file');
      assert.ok(paths.some(p => p.includes('.stories.tsx')), 'Should create stories file');
    });
  });
  ...
  describe('template validation', () => {
    ...
    it('should have all required templates', () => {
      ...
      requiredTemplates.forEach(template => {
        ...
        assert.ok(fs.existsSync(templatePath), `Should have ${template} template`);
      });
    });
    ...
    it('should have valid handlebars syntax in templates', () => {
      ...
      if (fs.existsSync(indexTemplate)) {
        ...
        assert.ok(content.includes('{{'), 'Should have handlebars placeholders');
  ...
  describe('component generation', () => {
    const testOutputDir = path.join(__dirname, '../fixtures/generated-components');
    ...
    it('should handle different component categories', () => {
      ...
      const categoryPrompt = generator.prompts.find(p => p.name === 'category');
      assert.ok(categoryPrompt, 'Should have category prompt');
      assert.ok(categoryPrompt.choices, 'Should have category choices');

      const expectedCategories = ['subatomic', 'atomic', 'molecular', 'organisms', 'templates'];
    ...
    it('should validate component name', () => {
      ...
      if (namePrompt.validate) {
        ...
        assert.ok(namePrompt.validate('ValidComponent'), 'Should accept valid PascalCase');
        assert.ok(!namePrompt.validate('invalid-component'), 'Should reject kebab-case');
        assert.ok(!namePrompt.validate('123Component'), 'Should reject names starting with numbers');
      }
    });
  });
...
export type { ComponentNameProps } from './ComponentName';`;
      ...
      assert.ok(result.includes('TestComponent'), 'Should include component name');
