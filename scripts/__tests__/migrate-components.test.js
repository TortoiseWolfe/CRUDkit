...
describe('migrate-components', () => {
  describe('module structure', () => {
    it('should export a function', () => {
      assert.strictEqual(typeof migrateComponents, 'function', 'migrate-components should export a function');
    });

    it('should accept options parameter', () => {
      assert.doesNotThrow(() => {
        if (migrateComponents) migrateComponents({ path: 'src/components', dryRun: true });
      });
    });
  });
  ...
  describe('migration process', () => {
    ...
    beforeEach(() => {
      ...
      fs.writeFileSync(path.join(missingIndexDir, 'MissingIndex.tsx'), 'export default function MissingIndex() {}');
      fs.writeFileSync(path.join(missingIndexDir, 'MissingIndex.test.tsx'), 'test("renders", () => {});');
      fs.writeFileSync(path.join(missingIndexDir, 'MissingIndex.stories.tsx'), 'export default { title: "MissingIndex" };');

      // Component missing test file
      const missingTestDir = path.join(testDir, 'MissingTest');
      fs.mkdirSync(missingTestDir, { recursive: true });
      fs.writeFileSync(path.join(missingTestDir, 'index.tsx'), 'export { default } from "./MissingTest";');
      fs.writeFileSync(path.join(missingTestDir, 'MissingTest.tsx'), 'export default function MissingTest() {}');
      fs.writeFileSync(path.join(missingTestDir, 'MissingTest.stories.tsx'), 'export default { title: "MissingTest" };');
      ...
      fs.writeFileSync(path.join(minimalDir, 'MinimalComponent.tsx'), 'export default function MinimalComponent() {}');
    });
    ...
    it('should identify components needing migration', () => {
      ...
      assert.strictEqual(result.toMigrate.length, 3, 'Should identify 3 components needing migration');
    });
    ...
    it('should create missing index.tsx files', () => {
      ...
      assert.ok(content.includes('export { default } from'), 'Index should have correct export');
      assert.ok(content.includes('MissingIndex'), 'Index should reference component name');
    });
    ...
    it('should create missing test files', () => {
      ...
      const testPath = path.join(testDir, 'MissingTest', 'MissingTest.test.tsx');
      assert.ok(fs.existsSync(testPath), 'Should create missing test file');

      const content = fs.readFileSync(testPath, 'utf8');
      assert.ok(content.includes('describe'), 'Test should have describe block');
      assert.ok(content.includes('MissingTest'), 'Test should reference component name');
      assert.ok(content.includes('renders'), 'Test should have basic render test');
    });
    ...
    it('should create missing story files', () => {
      ...
      const storyPath = path.join(testDir, 'MinimalComponent', 'MinimalComponent.stories.tsx');
      assert.ok(fs.existsSync(storyPath), 'Should create missing story file');

      const content = fs.readFileSync(storyPath, 'utf8');
      assert.ok(content.includes('export default'), 'Story should have default export');
      assert.ok(content.includes('title:'), 'Story should have title');
      assert.ok(content.includes('MinimalComponent'), 'Story should reference component name');
    });
    ...
    it('should respect dry-run mode', () => {
      ...
      const indexPath = path.join(testDir, 'MissingIndex', 'index.tsx');
      assert.ok(!fs.existsSync(indexPath), 'Should NOT create files in dry-run mode');
  ...
  describe('backup functionality', () => {
    ...
    beforeEach(() => {
      ...
      fs.writeFileSync(path.join(componentDir, 'TestComponent.tsx'), 'export default function TestComponent() {}');
    });
  ...
  describe('error handling', () => {
    it('should handle write permission errors', () => {
      ...
      assert.doesNotThrow(() => {
        const result = migrateComponents({ path: '/root/protected', dryRun: true });
        assert.ok(result.error || result.toMigrate.length === 0, 'Should handle permission errors');
      });
    });
    ...
    it('should handle invalid component names', () => {
      ...
      fs.writeFileSync(path.join(invalidDir, 'component.tsx'), 'export default function Component() {}');

      try {
        const result = migrateComponents({ path: testDir });
        assert.ok(result.skipped && result.skipped.length > 0, 'Should skip invalid components');
      } finally {
        fs.rmSync(testDir, { recursive: true, force: true });
      }
    });
  ...
  describe('reporting', () => {
    it('should generate migration report', () => {
      ...
      const result = migrateComponents({ path: 'src/components', dryRun: true });

      assert.ok(result.report, 'Should generate report');
      assert.ok(typeof result.report.migrated === 'number', 'Should count migrated components');
      assert.ok(typeof result.report.failed === 'number', 'Should count failed migrations');
      assert.ok(typeof result.report.skipped === 'number', 'Should count skipped components');
    });
    ...
    it('should log progress during migration', () => {
      ...
      try {
        ...
        assert.ok(logs.some(log => log.includes('Migrating')), 'Should log migration progress');
      } finally {
  ...
  describe('template generation', () => {
    it('should use correct template for index files', () => {
      ...
      assert.ok(result.includes('ButtonComponent'), 'Should include component name');
    ...
    it('should use correct template for test files', () => {
      ...
      assert.ok(result.includes('CardComponent'), 'Should include component name');
    ...
    it('should use correct template for story files', () => {
      ...
      assert.ok(result.includes('ModalComponent'), 'Should include component name');
