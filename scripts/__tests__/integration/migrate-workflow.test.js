...
describe('migration workflow integration', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(comp1Dir, 'ComponentOne.tsx'), 'export default function ComponentOne() {}');
    fs.writeFileSync(path.join(comp1Dir, 'ComponentOne.test.tsx'), 'test("renders", () => {});');
    fs.writeFileSync(path.join(comp1Dir, 'ComponentOne.stories.tsx'), 'export default { title: "ComponentOne" };');
    ...
    fs.writeFileSync(path.join(comp2Dir, 'index.tsx'), 'export { default } from "./ComponentTwo";');
    fs.writeFileSync(path.join(comp2Dir, 'ComponentTwo.tsx'), 'export default function ComponentTwo() {}');
    ...
    fs.writeFileSync(path.join(comp3Dir, 'ComponentThree.tsx'), 'export default function ComponentThree() {}');
  });
  ...
  it('should complete full migration workflow', () => {
    ...
    assert.strictEqual(auditBefore.summary.compliant, 0, 'Should have 0 compliant initially');
    assert.strictEqual(auditBefore.summary.nonCompliant, 3, 'Should have 3 non-compliant initially');
    ...
    assert.strictEqual(migrationResult.migrated, 3, 'Should migrate 3 components');

    // Step 3: Audit after migration
    const auditAfter = auditComponents({ path: componentsPath });
    assert.strictEqual(auditAfter.summary.compliant, 3, 'Should have 3 compliant after migration');
    assert.strictEqual(auditAfter.summary.nonCompliant, 0, 'Should have 0 non-compliant after migration');

    // Step 4: Validate structure
    const validation = validateStructure({ path: componentsPath });
    assert.strictEqual(validation.valid, true, 'Structure should be valid after migration');
  });
  ...
  it('should create proper backup before migration', () => {
    ...
    assert.ok(!fs.existsSync(path.join(backupComp1, 'index.tsx')),
  ...
  it('should handle dry-run mode correctly', () => {
    ...
    assert.strictEqual(dryRunResult.planned.length, 3, 'Should plan 3 migrations');

    // Verify no actual changes were made
    const comp1Index = path.join(componentsPath, 'atomic/ComponentOne/index.tsx');
