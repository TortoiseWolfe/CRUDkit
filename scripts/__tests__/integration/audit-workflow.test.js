...
describe('audit workflow integration', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(buttonDir, 'index.tsx'), 'export { default } from "./Button";');
    fs.writeFileSync(path.join(buttonDir, 'Button.tsx'), 'export default function Button() {}');
    fs.writeFileSync(path.join(buttonDir, 'Button.test.tsx'), 'describe("Button", () => {});');
    fs.writeFileSync(path.join(buttonDir, 'Button.stories.tsx'), 'export default { title: "Button" };');

    // Atomic components (some non-compliant)
    const cardDir = path.join(componentsDir, 'atomic/Card');
    fs.mkdirSync(cardDir, { recursive: true });
    fs.writeFileSync(path.join(cardDir, 'Card.tsx'), 'export default function Card() {}');
    // Missing other files - non-compliant

    // Molecular components
    const formDir = path.join(componentsDir, 'molecular/Form');
    fs.mkdirSync(formDir, { recursive: true });
    fs.writeFileSync(path.join(formDir, 'Form.tsx'), 'export default function Form() {}');
    fs.writeFileSync(path.join(formDir, 'Form.stories.tsx'), 'export default { title: "Form" };');
  ...
  it('should perform complete audit of project structure', () => {
    ...
    assert.strictEqual(result.summary.compliant, 1, 'Should find 1 compliant (Button)');
    assert.strictEqual(result.summary.nonCompliant, 2, 'Should find 2 non-compliant');
  });
  ...
  it('should generate detailed report with all findings', () => {
    ...
    assert.strictEqual(button.status, 'compliant', 'Button should be compliant');

    // Check Card is non-compliant
    const card = result.components.find(c => c.name === 'Card');
    assert.ok(card, 'Should find Card component');
    assert.strictEqual(card.status, 'non_compliant', 'Card should be non-compliant');
    assert.ok(card.missing.includes('index.tsx'), 'Card should be missing index');
    assert.ok(card.missing.includes('Card.test.tsx'), 'Card should be missing test');

    // Check Form is non-compliant
    const form = result.components.find(c => c.name === 'Form');
    assert.ok(form, 'Should find Form component');
    assert.strictEqual(form.status, 'non_compliant', 'Form should be non-compliant');
  });
  ...
  it('should handle nested component structures', () => {
    ...
    const nestedDir = path.join(testProjectDir, 'src/components/organisms/Dashboard/widgets/Widget');
    fs.mkdirSync(nestedDir, { recursive: true });
    fs.writeFileSync(path.join(nestedDir, 'Widget.tsx'), 'export default function Widget() {}');
  ...
  it('should respect ignore patterns', () => {
    ...
    fs.writeFileSync(path.join(testsDir, 'utils.test.js'), 'test("util", () => {});');
    ...
    const hasTestUtils = result.components.some(c => c.name.includes('utils'));
